 ...
 class Toolbar extends React.Component {
   ...
   static _defaultItemOptions(element, intl) {
     switch (element) {
       ...
       case 'Tags':
         return [{ value: 'place_holder_option_1', text: intl.formatMessage({ id: 'place-holder-option-1' }), key: `${ID.uuid()}` }];
       case 'Checkboxes':
         return [
           { value: 'place_holder_option_1', text: intl.formatMessage({ id: 'place-holder-option-1' }), key: `${ID.uuid()}` },
           { value: 'place_holder_option_2', text: intl.formatMessage({ id: 'place-holder-option-2' }), key: `${ID.uuid()}` },
       ...
       case 'RadioButtons':
         return [
           { value: 'Yes', text: intl.formatMessage({ id: 'Yes' }), key: `${ID.uuid()}` },
           { value: 'No', text: intl.formatMessage({ id: 'No' }), key: `${ID.uuid()}` },
         ];
       default:
         return [];
     }
   ...
   create(item) {
     ...
     if (item.logicEnable) { elementOptions.logicEnable = item.logicEnable; }
 
     elementOptions.canHaveLogicBlock = item.canHaveLogicBlock !== false;
   ...
   render() {
     return (
       <div className="col-md-12 react-form-builder-toolbar float-left">
         {/* <h4>{this.props.intl.formatMessage({ id: 'toolbox' })}</h4> */}
         <ul>
           {
             this.state.items.map((item) => (<ToolbarItem data={item} key={item.key} onClick={this.create} onCreate={this.create} />))
           }
         </ul>
       </div>
     );
